...
aapl_data <- getSymbols("AAPL", src = "yahoo", 
                        from = "2014-01-01", 
                        to = "2025-02-01", 
                        auto.assign = FALSE)


price_data <- Cl(aapl_data)
...
results_tbl <- tibble(
  ...
  Actual = round(as.numeric(test_data), 2), 
...
ggplot(results_tbl, aes(x = Date)) +
  ...
  # X-Axis: Include all days in January (Daily breaks)
  scale_x_date(
    date_breaks = "1 day", 
    date_labels = "%d",
    limits = c(as.Date("2025-01-01"), as.Date("2025-01-31")),
    expand = expansion(mult = 0.05) # Add small padding
  ) +
  
  # Y-Axis: Continuous and formatted (Currency style)
  scale_y_continuous(
    n.breaks = 10,  # Ensure more ticks for "more continuous" feel
    labels = scales::dollar_format()
  ) +
  
  # Labels and Theme
  labs(
    title = "APPLE Stock Prediction: January 2025",
    x = "Day of January",
    color = "Legend"
  ) +
  theme_minimal() +
